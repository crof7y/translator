...
#Replace 'audio-file.flac' in the next line with desired audio to translate (within the same directory)
with open(join(dirname(__file__), 'audio-file.flac'), 
          'rb') as audio_file:
    speech_recognition_results = speech_to_text.recognize(
        audio=audio_file,
        content_type='audio/flac',
        #Change audio type ^^ if audio is type other than flac (ex: audio/wav)
...
#This block creates audio synthesized translation and writes to a file 's2s_output.wav'
